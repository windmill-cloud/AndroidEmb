...
//        final MyGdxGame game1= new MyGdxGame();
        ...
        tc.setEnabled(false);
        CBT.setSelection(1);
        CBT.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String check = CBT.getSelectedItem().toString();
                if (check.equals("6 Band")) {
                    tor.setEnabled(true);
                    tc.setEnabled(true);
                    Log.d("INFO: \t", "check 6 " + "1");

                }
                if (check.equals("5 Band")) {
                    tor.setEnabled(true);
                    tc.setEnabled(false);
                }
                if (check.equals("4 Band")) {
                    tor.setEnabled(true);
                    tc.setEnabled(false);
                }
                if (check.equals("3 Band")) {
                    tor.setEnabled(false);
                    tc.setEnabled(false);
                }
...
//                }
    ...
    public static int[] ResCal(double rss, int[] flag) {
        ...
        if (flag[2] == 4) {
            if (rss2 > 0) {
                ...
                int rss4=(int)(long)rss3;
                color[0] = (rss4 / 10) % 10;
                Log.d("INFO", "color 0" + color[0]);
                color[1] = rss4 % 10;
                Log.d("INFO", "color 1" + color[1]);
    ...
    public static int[] getttc(String tor, String tc, String cbt) {
        ...
        if (cbt.equals("6 Band")) {
            flagout[2] = 6;
        }
        if (cbt.equals("5 Band")) {
            flagout[2] = 5;
        }
        if (cbt.equals("4 Band")) {
            flagout[2] = 4;
        }
        if (cbt.equals("3 Band")) {
            flagout[2] = 3;
        }
